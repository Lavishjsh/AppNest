...
//
//  Imports rootless-jailbreak .deb tweak packages into a LiveContainer/AppNest tweak
//  folder, reusing the existing ar/tar extraction (unarchive.m -> libarchive) and MachO
//  rpath-patching code already used for manually-imported dylib/framework tweaks.
//
//  Adapted from Vishram1123/LiveContainer's feature/deb-tweak-import branch. Signing is
//  intentionally not handled here: LCUtils.signTweaks already walks a tweak folder
//  recursively (LCUtilsExtensions.swift), so a deb-imported package's nested dylibs are
//  discovered and signed the same way as any other tweak, with no separate "sign this
//  package" step.
//
...
enum DebImporter {
    ...
    // user created themselves to group tweaks (LCTweaksView.swift shows a different icon
    // for it, and reuses this same marker to know it's safe to overwrite on re-import of
    // the same package, and to clean up its redirect/jbroot bookkeeping on delete).
    ...
    static func importDeb(at debUrl: URL, into destinationFolder: URL) throws -> DebImportResult {
        ...
        // control metadata must not fail the import.
    ...
    // stays intact -- TweakLoader.m's recursive loader already finds .dylib/.framework
    // at arbitrary depth, so nesting doesn't stop it from finding them. Anything the
    ...
    private static func installPayload(from dataRoot: URL, into destination: URL, tweakName: String) throws -> [String] {
        ...
        if fm.fileExists(atPath: tweakDir.path, isDirectory: &existingIsDir) {
            ...
            // themselves, which a same-named .deb would otherwise silently wipe out with
            // no confirmation.
        ...
        // resolving symlinks on both sides independently before comparing is what makes
        // the comparison correct regardless of which side (if either) needed it.
        //
        ...
        //
        // The "where we put it" side is stored *relative to destination* (the folder this
        // .deb was imported into), not as an absolute path: that folder lives inside
        // AppNest's own app container or the shared app-group container, and both of those
        // roots can change out from under an already-imported tweak (AppNest being
        // reinstalled/updated gets a brand new sandbox container UUID, and "convert app to
        // shared" physically moves the whole tweak folder from one root to the other). An
        // absolute path baked in at import time goes stale the moment either happens,
        // silently breaking every redirect until the tweak is re-imported. Storing a
        // root-relative path and letting DebPathRedirect.m re-resolve it against whichever
        // root is actually current at each launch keeps the mapping valid across both.
        //
        ...
        // (<destination>/<tweakName>/...), so a ".jbroot" pointing at just *that* tweak's
        // own subfolder would resolve same-package dependencies but never find another
        // package's files. Instead, every tweak's ".jbroot" points at one shared location,
        // <destination>/.lc_shared_jbroot -- a flat mirror (see rebuildSharedJbroot) of
        // every framework/bundle across every currently-imported tweak *in this same
        // folder*, keyed by original absolute path -- so cross-package and same-package
        // dependencies resolve the same way. It's rebuilt on every import, so import order
        // doesn't matter. This does NOT help a dependency on a real system library that was
        // never shipped in any .deb (e.g. libroothide.dylib itself) -- there's nothing on
        // disk for the mirror to point at.
    ...
    // Rebuilds <destination>/.lc_shared_jbroot from scratch as a flat mirror of every
    // framework/bundle any deb-imported tweak under `destination` has ever registered a
    // redirect for -- the merged .lc_deb_redirects.plist is already exactly that data,
    // keyed by the resource's original absolute path, so this just re-expresses each
    // bare-path entry (skipping the "/var/jb"-prefixed duplicate of the same entry, and the
    // "id:"-prefixed identifier entries, which aren't filesystem paths) as a symlink at the
    // matching location under .lc_shared_jbroot. Every tweak's own ".jbroot" symlink (see
    // createJbrootSymlink) and the extra rpath LCPatchAddRPath adds both point at this fixed
    // location, so refreshing its contents here is all that's needed to pick up a
