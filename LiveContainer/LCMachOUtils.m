...
void LCPatchAddRPath(const char *path, struct mach_header_64 *header) {
    ...
    // DebImporter.swift maintains <destination>/.lc_shared_jbroot as a flat mirror --
    // symlinks keyed by each resource's original absolute path -- of every framework/
    // dylib across every currently-imported tweak in that same folder, rebuilt each time
    // any tweak is (re)imported. See DebImporter.swift's rebuildSharedJbroot for the
