...
// put that resource inside the tweak folder it was imported into. Path keys start with
// "/"; entries prefixed "id:" instead map a bundle's own CFBundleIdentifier to its path,
// for tweaks that look their bundle up by identifier rather than by path.
//
// This is a separate mechanism from TweakLoader.m's own gCurrentTweakResourcePath /
// redirectTweakResourcePath: that one rewrites lookups made *after* a bundle/path is
// already resolved, relative to wherever the requesting dylib itself sits, which works
// for tweaks whose resources sit flat alongside the dylib. A deb payload mirrors its
// original (possibly deeply nested) directory tree instead, so this table -- built once
// at import time from every resource's *original* absolute path -- is what makes that
// tree's absolute-path references resolve, independent of where the referencing dylib
...
// itself lives in), not as absolute paths -- that root can move (AppNest reinstalled with
// a new sandbox container UUID, or the tweak folder relocated by "convert to shared"), so
// the absolute destination is reconstructed fresh against whichever root is current at
// each launch instead of being baked in at import time. See DebImporter.swift for the
// write side of this.
...
// calls, the same way TweakLoader.m's own NSBundle/NSFileManager hooks work elsewhere in
// this file.
